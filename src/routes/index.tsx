...
function SectionHeading({ letter, title }: { letter: string; title: string }) {
  return (
    <Reveal className="flex items-center gap-3 border-b border-border pb-2">
      <span className="font-meta text-[10px] font-semibold tracking-[0.16em] text-primary">
        ({letter})
      </span>
...
function Panel({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className={`border-l-2 border-primary bg-card p-4 md:p-5 ${className}`}>
      {children}
    </div>
  );
}
...
function Index() {
  return (
    <div className="min-h-screen bg-background font-body text-foreground antialiased">
      ...
      {/* sticky navigation */}
      <header className="sticky top-0 z-50 border-b border-border bg-card/95 backdrop-blur-sm print:hidden">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-5 py-3">
          <div className="flex items-baseline gap-2">
            <span className="font-display text-sm font-bold">M. Tsiamo</span>
      ...
      <main className="mx-auto max-w-4xl bg-card px-5 pb-16 shadow-sm ring-1 ring-border md:my-8 md:px-12 md:pb-12 print:my-0 print:shadow-none print:ring-0">
        {/* hero */}
        <section className="border-b border-border pt-10 pb-9 md:pt-14 md:pb-11">
          <Reveal className="grid items-start gap-x-6 sm:grid-cols-[auto_1fr]">
            <div
              aria-hidden="true"
              className="grid size-16 place-items-center rounded-sm bg-primary text-primary-foreground sm:row-span-4 md:size-20"
            >
              <span className="font-display text-xl font-bold md:text-2xl">
                MT
              </span>
            </div>
            <p className="rise mt-5 text-[10px] font-semibold uppercase tracking-[0.16em] text-primary sm:mt-0 [animation-delay:120ms]">
              South Africa · Open to opportunities
            </p>
            <h1 className="rise mt-2 font-display text-3xl font-bold leading-tight text-balance [animation-delay:200ms] md:text-5xl">
              Musekwa Tsiamo Precious
            </h1>
            <p className="rise mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground text-pretty [animation-delay:300ms] md:text-[15px]">
            ...
            <div className="rise mt-5 flex flex-wrap items-center gap-4 [animation-delay:420ms]">
              <a
                href="#contact"
                className="rounded-sm bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground transition-colors hover:bg-foreground"
              >
                Get in touch
              </a>
              <a
                href="#about"
                className="border-b border-foreground pb-1 text-xs font-semibold text-foreground transition-colors hover:text-primary"
              >
                View profile
              </a>
            </div>
              ...
              <Panel>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {TECHNICAL_SKILLS.map((skill) => (
                    <span
                      key={skill}
                      className="rounded-sm bg-muted px-2.5 py-1.5 text-xs font-medium"
                    >
                      {skill}
                    </span>
                  ))}
            ...
            {PROJECTS.map((project, index) => (
              <Reveal key={index} delay={140 + index * 100}>
                <div
                  className={
                    project.placeholder
                      ? "h-full border border-dashed border-border bg-card p-4"
                      : "h-full border-t-2 border-primary bg-card p-4 shadow-sm ring-1 ring-border"
                  }
                >
        ...
        <section id="certifications" className="py-8">
          ...
          <Reveal delay={140} className="mt-4">
            <div className="border-l-2 border-primary bg-card p-4 md:p-5">
              ...
              <Reveal key={row.label} delay={140 + index * 100}>
                <a
                  href={row.href ?? "#contact"}
                   className="flex items-center justify-between border-b border-border bg-card px-1 py-3 transition-colors hover:bg-muted md:px-3"
                >
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                      {row.label}
